...
def create_single_subject_log_callback(training_result: NeuralNetworkSingleSubjectTrainingResult):
...
if __name__ == "__main__":
    ...
    for subject_id in all_subject_ids:
        ...
        repetition_folds = generate_repetitions_folds(all_repetitions=np.array([1,2,3,4,5,6,7,8]))
        for repetition_fold in repetition_folds:
            
            (train_measurements, validate_measurements, test_measurements) = split_by_sets(subject_measurements, 
                train_sets=repetition_fold['train_reps'], validate_sets=repetition_fold['validate_reps'], test_sets=repetition_fold['test_reps'])
            ...
            test_measurements = apply_standardization_params(test_measurements, standardization_mean, standardization_std)
            ...
            single_subject_log_callback = create_single_subject_log_callback(training_result)
            ...
            model.fit(dataloader_dictionary, num_epochs, adam_learning_rate, adam_weight_decay, verbose=True, 
                    model_checkpoint=model_checkpoint, training_log_callback=single_subject_log_callback)
            
            # test        
            pre_model_state, pre_model_config = model_checkpoint.load_best_model_config()
            model.load_state_dict(pre_model_state)
            pre_classifier = EMGClassifier(None)
            pre_classifier.model = model
            predicted_classes, class_probabilities = pre_classifier.run(test_windows)
