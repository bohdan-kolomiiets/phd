...
class CNN(nn.Module):
    def __init__(self, n_output, n_channels, n_samples, n_filters=256):
        ...
        CNN.__try_move_to_accelerator(self)
    ...
    @staticmethod
    def __try_move_to_accelerator(obj):
    ...
    def fit(self, dataloader_dictionary, learning_rate=1e-3, num_epochs=100, verbose=True):
        ...
        for epoch in range(num_epochs):
            ...
            for data, labels in dataloader_dictionary["training_dataloader"]:
                optimizer.zero_grad()
                data = CNN.__try_move_to_accelerator(data)
                labels = CNN.__try_move_to_accelerator(labels)
    ...
    def predict(self, x):
        ...
        x = CNN.__try_move_to_accelerator(x)
        y = self.forward(x)
